...
import { Plus, Download, Copy, Trash2, FileText } from 'lucide-react';
...
import { InvoiceStatus } from '@/lib/factuur/types/invoice';
...
export default function InvoicesPage() {
  ...
  const handleDuplicate = (invoiceNumber: string) => {
    alert(`Dupliceren van factuur ${invoiceNumber}`);
  };

  const handleDelete = (invoiceNumber: string) => {
    if (confirm(`Weet je zeker dat je factuur ${invoiceNumber} wilt verwijderen?`)) {
      alert(`Factuur ${invoiceNumber} verwijderd`);
    }
  };
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {mockInvoices.filter(inv => inv.status === 'verzonden').length} facturen
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {mockInvoices.filter(inv => inv.status === 'betaald').length} facturen
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="text-3xl font-bold text-gray-600">
                {mockInvoices.filter(inv => inv.status === 'concept').length}
              </div>
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => handleDuplicate(invoice.invoiceNumber)}
                            title="Dupliceren"
                          >
