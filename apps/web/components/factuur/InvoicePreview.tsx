    ...
    <div className="bg-card border rounded-lg p-8 shadow-sm">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <div className="text-sm text-muted-foreground whitespace-pre-line">{invoice.company.address}</div>
          {invoice.company.email && (
            <div className="text-sm text-muted-foreground">{invoice.company.email}</div>
          )}
          {invoice.company.phone && (
            <div className="text-sm text-muted-foreground">{invoice.company.phone}</div>
          )}
        </div>
        <div className="text-right">
          <h1 className="text-3xl font-bold mb-2">FACTUUR</h1>
          <div className="text-sm space-y-1">
            <div><span className="text-muted-foreground">Nummer:</span> {invoice.invoiceNumber}</div>
            <div><span className="text-muted-foreground">Datum:</span> {formatDate(invoice.date)}</div>
            <div><span className="text-muted-foreground">Vervaldatum:</span> {formatDate(invoice.dueDate)}</div>
          </div>
        </div>
      </div>
      ...
      {/* Client Info */}
      <div className="mb-8">
        <div className="text-sm font-semibold text-muted-foreground mb-2">FACTUURADRES</div>
